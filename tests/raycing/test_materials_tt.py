...
#        p6, = ax.plot((theta - thetaCenter) * convFactor, abs(curP)**2, '--g')
        ...
        curSD, curPD = crystal.get_amplitude(E, gamma0, gammah, hns0,
                                             alphaAsym=alpha, ucl=matCL,
                                             useTT=True, Rcurvmm=Rcurv)
        print "OpenCL calculation for S- and P- takes", time.time()-t0, "s"
# phases:
        ax2 = ax.twinx()
        ...
        if geom.endswith('transmitted'):
            geomPrefix += 't'
        fig.suptitle(r'{0} Si{1}, $\alpha={2:.1f}$ deg, bending R={3:.2f}m'.format(geom,
                     hkl, alphaDeg, Rcurv/1e3), fontsize=16)
        ...
        p11, = ax.plot((theta - thetaCenter) * convFactor, abs(curSD)**2, '-g')
        p12, = ax.plot((theta - thetaCenter) * convFactor, abs(curPD)**2, '--g')
...
#        'pxrt-Authier', 'pxrt-Bel&Dm'], loc=1)
        ax2.legend([p1, p3, p7, p11], ['XCrystal/XOP', 'XInpro/XOP', 'xrt',
                                       'xrt TT'],
                   loc=legendPos2)
        ax2.add_artist(l1)
    ...
    if hkl == '111':  # Si111
        if geom.startswith('Bragg'):
            ...
            dtheta = np.linspace(-150, 150, 1500) * 1e-6
        dSpacing = 3.13562
        hklInd = 1, 1, 1
    elif hkl == '333':  # Si333
        if geom.startswith('Bragg'):
...
#    compare_rocking_curves('333', t=0.100, geom='Bragg transmitted')
    for revR in np.linspace(-1, 1, 21): 
