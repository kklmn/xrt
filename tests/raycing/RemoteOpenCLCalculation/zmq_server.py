...
a local computer. 
Users can seamlessly execute their scripts in their favorite IDE,
offloading resource-intensive calculations to a remote server over the network.
The only trade-off is the potential delay due to data transfer, which is
outweighed by the benefits when local computations take longer than data
transfer time. Furthermore, the local graphical user interface (GUI) remains
responsive without freezes or issues caused by high GPU/CPU loads.
This script now supports all acceleration scenarios:
sycnhrotron sources, wave propagation, bent crystals and multilayers
reflectivity.

Script Components
The GPU accelerator script is comprised of two files:
zmq_server.py: The server script serves as the main component, responsible for
receiving data and kernel names from the client. It listens on a predefined
port, processes the received package, executes the specified kernel on the GPU,
and sends the computed data back to the client. This server script can be
executed independently or in conjunction with the queue manager.

queue_device.py: The queue manager script facilitates the handling of multiple
user requests and the distribution of computational tasks across multiple
servers. It provides scalability and load balancing capabilities. The queue
manager can be executed on the same machine as the server or on a dedicated
node. However, when running the queue manager on a separate node, data transfer
times may increase.
...
# Replace 'localhost' with the queue manager address if not run on same machine
