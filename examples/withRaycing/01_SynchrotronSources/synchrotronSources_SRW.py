...
from xrt.backends.raycing.physconsts import FINE_STR, SIE0
...
#        eEpsilonX=0.263, eEpsilonZ=0.008)  # Emittance [nmrad]
    xlimits = [-2, 2]  # Horizontal limits of the plot [mm]
    zlimits = [-2, 2]  # Vertical limits of the plot [mm]
    xlimitsZoom = [-0.4, 0.4]
    zlimitsZoom = [-0.4, 0.4]
...
if energyRange == 'mono':
    eMinRays, eMaxRays = E0*0.9995, E0*1.0005
    eUnit = 'eV'
...
def build_beamline(nrays=1e5):
    ...
    beamLine.source = Source(
        ...
        eN=1, nx=bins, nz=bins,
#        eN=1000, nx=40, nz=20,
        **kwargs)
#    beamLine.source.dimExy = (1, bins, bins)
...
def run_process(beamLine):
    ...
    if 'wave' in what and Source in (rs.Undulator, rs.UndulatorSRW):
