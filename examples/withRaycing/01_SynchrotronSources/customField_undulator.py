...
from xrt.backends.raycing.physconsts import E2W, CH, PI2, K2B
...
fieldEndsZ = np.linspace(-2*Nend*L0, 2*Nend*L0, int(4*Nend*L0)+1)
fieldEndsB = K2B * Kmax/L0 * np.sin(2*np.pi*fieldEndsZ/L0)
a = fieldEndsB*np.exp(-(fieldEndsZ)**2/np.abs((0.5*Nend*L0)**2))
#a = fieldEndsB*np.exp(-np.abs(fieldEndsZ)/np.abs((0.5*Nend*L0)**2))
lenA = len(a)
fieldZ = np.hstack((fieldEndsZ[:int(lenA//2)]+zMin, fieldZ, fieldEndsZ[int(lenA//2+1):]+zMax))
fieldB = np.hstack((a[:int(lenA//2)], fieldB, a[int(lenA//2+1):]))
...
if sourceType == 'u':
    ...
    pprefix = 'Custom_NF_SFF_66920eV_neg_noEnd_noAutoReduce'
...
#        eSigmaX=48.65, eSigmaZ=6.197,  # Size of the electron beam [mkm]
        eSigmaX=0, eSigmaZ=0,  # Size of the electron beam [mkm]
...
#         eSigmaX=0., eSigmaZ=0.,  # Zero size electron beam
         ...
         R0=R0,   # Near Field.
#         gridLength = 512*2,
#         gIntervals=2, gNodes=512000, # Number of the integration intervals. Should be
        # increased for the near field and custom magnetic field cases.
         # 77 is ok for 0.5eV
#         gp = 1e-5,
#         gp=1e-6,  # Precision of the integration.
#        targetOpenCL=(0, 0),
        targetOpenCL='192.168.2.119:15559',
#        targetOpenCL='GPU',
...
#        customField = CFfileName,
        xPrimeMax = (screenMaxX+abs(screenOffsetX))/3.,
        zPrimeMax = screenMaxZ/3.,
#        nRK=10,
#        zPrimeMax = 2,
...
#    print(kwargs)
    ...
    print("Init done")
#    beamLine.source.gIntervals = Np*2
    print("source.gIntervals =", beamLine.source.gIntervals)
    print('source done')
...
def run_process(beamLine):
    startTime = time.time()
    kw = {}
    print("RR source.gIntervals =", beamLine.source.gIntervals)
#    kw = beamLine.source.kwLoop
    if 'mono' in prefix and isInternalSource:
        kw['fixedEnergy'] = E0

#    if 'wave' in what and Source == rs.Undulator:
    if 'wave' in what and Source in [rs.SourceFromField, rs.Undulator]:
        if isSpherical:
