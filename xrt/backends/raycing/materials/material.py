...
class TXMMaterial(Material):
    ...
    The HDF5 file must contain an integer 3D dataset ``/indexGrid`` with axis
    order ``(z, y, x)`` and limit datasets ``/limits/x``, ``/limits/y`` and
    ``/limits/z`` in mm. The point counts are taken from the grid shape.
    ``materialTable`` maps integer material indices to material instances.
    """
    ...
    def __init__(
            self, fileName='', materialTable=None, name='',
            indexDataset='indexGrid', limitsGroup='limits', **kwargs):
        ...
        for ignored in self.hiddenParams:
            if ignored in kwargs:
                del kwargs[ignored]
        if materialTable is None:
            raise ValueError('TXMMaterial requires materialTable')
        Material.__init__(
            self, elements=(), quantities=(), kind='plate', rho=0, t=None,
            name=name, refractiveIndex=1., **kwargs)
        self.fileName = fileName
        self.indexDataset = indexDataset
        self.limitsGroup = limitsGroup
        self.materialTable = self._normalize_material_table(materialTable)
        self._read_volume_file()
        self._validate_material_table()
    ...
    def _normalize_material_table(self, materialTable):
        if isinstance(materialTable, dict):
            return {
                int(k): self._resolve_material_table_entry(v)
                for k, v in materialTable.items()}
        return {
            i: self._resolve_material_table_entry(v)
            for i, v in enumerate(materialTable)}

    def _resolve_material_table_entry(self, material):
    ...
    def _read_volume_file(self):
        ...
        with h5py.File(self.fileName, 'r') as h5:
            self.indexGrid = np.asarray(h5[self.indexDataset][:])
            axisOrder = h5[self.indexDataset].attrs.get('axisOrder', 'zyx')
            if isinstance(axisOrder, bytes):
                axisOrder = axisOrder.decode()
            if axisOrder.lower() != 'zyx':
                raise ValueError(
                    'TXMMaterial currently expects indexGrid axisOrder="zyx"')

            limits = h5[self.limitsGroup]
            self.xLimits = np.asarray(limits['x'][:], dtype=float)
            self.yLimits = np.asarray(limits['y'][:], dtype=float)
            self.zLimits = np.asarray(limits['z'][:], dtype=float)
            self.backgroundIndex = int(
                h5[self.indexDataset].attrs.get('backgroundIndex', 0))

        if self.indexGrid.ndim != 3:
            raise ValueError('indexGrid must be a 3D integer dataset')

        self.nz, self.ny, self.nx = self.indexGrid.shape
        self.dx = (self.xLimits[1] - self.xLimits[0]) / self.nx
        self.dy = (self.yLimits[1] - self.yLimits[0]) / self.ny
        self.dz = (self.zLimits[1] - self.zLimits[0]) / self.nz
        self.zEdges = np.linspace(
            self.zLimits[0], self.zLimits[1], self.nz + 1)

    def _validate_material_table(self):
        missing = set(np.unique(self.indexGrid)) - set(self.materialTable)
        if missing:
            raise ValueError(
                'materialTable has no entries for indices {0}'.format(
                    sorted(int(v) for v in missing)))
        for index, material in self.materialTable.items():
            if not hasattr(material, 'get_refractive_index'):
                raise TypeError(
                    'materialTable[{0}] has no get_refractive_index method'
                    .format(index))
    ...
    def _get_refractive_index_by_indices(self, E, indices):
        ...
        for index in np.unique(flatIndices):
            ...
            material = self.materialTable[int(index)]
    ...
    def get_refractive_index(self, E, x=None, y=None, z=None):
        if x is None or y is None or z is None:
            material = self.materialTable[self.backgroundIndex]
