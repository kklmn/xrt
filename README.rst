...
Optional components can be installed depending on the intended use:

Graphical interface (xrtQook and xrtGlow):

.. code-block:: bash

    pip install xrt[pyqt5]

EPICS integration:

.. code-block:: bash

    pip install xrt[epics]
...
After installation, the graphical interface can be started with:
...
For detailed installation instructions and platform-specific notes,
...
Optional functionality requires:

- PyQt5 and Qt WebEngine (GUI tools)
- PyOpenCL (GPU/OpenCL acceleration)
- EPICS softIOC (beamline control integration)
...
The new propagation engine and GUI infrastructure are being stabilized.
Core functionality is operational, but some features are still being
restored or refined during the beta cycle. Feedback from users is
welcome.
...
xrt is distributed under the MIT License.
